...
let orderingThemeColors

const runOrderingWidget = ({container, numbers}) => {
    const getThemeColors = (theme) => {
        ...
        const themes = {
            "light": {
                numberColors: {normal: "#FFF", hoverAndClick: "#efb1b1"},
                spotColors: [175, 175],
            ...
            "dark": {
                numberColors: {normal: "#000", hoverAndClick: "#efb1b1"},
                spotColors: [75, 75],
    ...
    const setThemeColors = (theme) => {
        orderingThemeColors = getThemeColors(theme)
    }
...
document.body.clientHeight
    ...
    const sketch = p => {
        p.setup = () => {
            ...
            orderingCards = new OrderingCards(numbers, p, updateHiddenInputs)
        }

        p.draw = () => {
            p.background(orderingThemeColors.backgroundColor)

            if(p.mouseIsPressed) orderingCards.pressed()
            if(!p.mouseIsPressed) orderingCards.released()
        ...
        p.windowResized = () => {
            ...
            // Resize widgets
            orderingCards.resize()
        }

        p.touchMoved = (e) => {
            if(!e.cancelable) return 

            if(orderingCards.otherCardIsBeingDragged()) return false
        }
    }
...
class OrderingCards {
    constructor(array, p, updateHiddenInputs) {
        ...
        if(array.length < 2) throw "Error: There are less than 2 numbers in the array"

        this.output = new Array(this.array.length).fill(null)
        ...
        this.cardStart = this.p.createVector(
            this.p.width / 2 - (array.length * this.cardDims.width + (array.length - 1) * this.cardDims.gap) / 2,
        ...
        // Create all of the cards on the screen
        for(let i = 0; i < array.length; i++) {
            this.cards.push(this.makeCard({i, colors: orderingThemeColors.numberColors}))
        }
    }

    updateOutput() {
        const spotsOutputRowStart = this.p.floor(this.spots.length / 2)
  
        // Find the value of each spot in the output row
        for(let i = spotsOutputRowStart; i < this.spots.length; i++) {
            // Update the output with the value of the card in this spot, or if it's empty, with null
            this.output[i - spotsOutputRowStart] = this.spots[i].card != null ? this.spots[i].card.n : null
        }
    }

    makeCard({i, colors = {normal: "#FFF", hoverAndClick: "#CCC"}}) {
        return new Card(
            this.cardStart.x + (this.cardDims.gap + this.cardDims.width) * i, // Card x position
            this.cardStart.y,                                                 // Card y position
            this.cardDims.width,                                              // Card width
            this.cardDims.height,                                             // Card height
            this.cardDims.radius,                                             // Card corner radius
            this.array[i],                                                    // Card number
            this.spots[i],                                                    // The starting spot of this card
    ...
    makeSpot(i, j) {
        return new Spot(
            ...
            this.cardDims.radius,                                             // Card corner radius
            this.p,                                                           // Reference to the p5 sketch,
            orderingThemeColors.spotColors[j]
        )
    }
    ...
    resize() {
        ...
        this.cardStart = this.p.createVector(
            this.p.width / 2 - (this.array.length * this.cardDims.width + (this.array.length - 1) * this.cardDims.gap) / 2,
            this.p.height / 2 - this.cardDims.gap / 2 - this.cardDims.height
        )

        for(let j = 0; j < 2; j++) {
            for(let i = 0; i < this.array.length; i++) {
                this.spots[i + (this.array.length)*j].resize(
                    this.cardStart.x + (this.cardDims.gap + this.cardDims.width) * i, // Spot x position
                    this.cardStart.y + (this.cardDims.height + this.cardDims.gap) * j,// Spot y position
                    this.cardDims.width,
    ...
    draw() {
        ...
        const smallestTextPosition = this.getSmallestTextPosition()
        const largestTextPosition = this.getLargestTextPosition()

        this.p.textSize(this.cardDims.height/7)
        this.p.textAlign(this.p.LEFT, this.p.CENTER)
        this.p.fill(orderingThemeColors.textColor)
        this.p.noStroke()
        this.p.textStyle(this.p.BOLD)

        this.p.text("Smallest", smallestTextPosition.x, smallestTextPosition.y)
        this.p.text("Largest", largestTextPosition.x, largestTextPosition.y)
...
class Card {
    constructor(x, y, w, h, r, n, spot, spots, otherCardIsBeingDragged, p, colors) {
    ...
    show() {
        this.p.stroke(orderingThemeColors.borderColor)
        
        // Depending on the state of the card, fill it with the correct color
        if (this.dragging) {
            this.p.fill(orderingThemeColors.numberColors.hoverAndClick)
        } else if (this.mouseOver) {
            this.p.fill(orderingThemeColors.numberColors.hoverAndClick)
        } else {
            this.p.fill(orderingThemeColors.numberColors.normal)
        }
...
const showNumber = ({n, x, y, w, h, p, fontSize}) => {
    ...
    switch(numberCase) {
        case "integer": {
            p.noStroke()
            p.fill(orderingThemeColors.textColor)
        ...
        case "fraction": {
            ...
            p.stroke(orderingThemeColors.textColor)
...
class Spot {
    ...
    over() {
        ...
        if (this.checkEdges({left: this.x, right: this.x + this.w, top: this.y, bottom: this.y + this.h}) && this.checkCorners()) {
            ...
            if(this.card != null) this.p.cursor("pointer")
