let vennDiagramThemeColors;

const runVennDiagramWidget = ({container, labels, values, interactive, colors, coloredRegions}) => {
    ...
    const setThemeColors = (theme) => {
        vennDiagramThemeColors = getThemeColors(theme);
    }
...
class VennDiagram {
    ...
    draw() {
        var p = this.p;
        var bg = vennDiagramThemeColors.backgroundColor;
        ...
        p.fill(vennDiagramThemeColors.labelTextColor)
        p.noStroke();
        ...
        if (this.values[0]) {
            if (!this.values[0].type) p.text(this.values[0], p.width / 4, p.height / 2)
                ...
                p.push();
                p.stroke(vennDiagramThemeColors.labelTextColor);
