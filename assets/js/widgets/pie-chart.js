let pieChartThemeColors

const runPieChartWidget = ({container, segments, interactive, pieColors}) => {
    ...
    pieChartThemeColors = getThemeColors(defaultTheme)
    
    const setThemeColors = (theme) => {
        pieChartThemeColors = getThemeColors(theme)
    }
...
document.body.clientHeight
    ...
    const updateHiddenInputs = (output) => {
        answerHiddenInput.value = encodeURIComponent(JSON.stringify(output.map(segment => segment.value)))
    }
    ...
    const sketch = p => {
        ...
        p.draw = () => {
            p.background(pieChartThemeColors.backgroundColor)
            pieChart.draw()
        }
        ...
        p.touchMoved = (e) => {
            ...
            if(pieChart.otherPointIsDragging()) return false
        }
    }
...
class PieChart {
    ...
    drawChart() {
        ...
        for(let i = 0; i <= this.points.length; i++) {
            ...
            if(this.interactive) this.updateHiddenInputs(this.output)
        ...
        for(let i = 0; i < this.segments.length; i++) {
            ...
            // Style the text
            this.p.fill(pieChartThemeColors.labelTextColor)
