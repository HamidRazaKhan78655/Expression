...
const runCompoundInterestWidget = ({ 
	container, 
	inputType, 
	...
	const returnSteps = ({ value1, value2 }) => {
		...
		const steps = [
			expression, 
			`Compound interest means that each time interest is paid onto an amount saved or owed, the added interest also receives interest from then on.<br>Put simply, compound interest changes the amount of money in the bank each time and a new calculation has to be worked out.<br><br>Year 1: £\\(${value1}\\) + \\(${value2}\\)%`, 
			`\\( = £${value1} + £${(value1 * (value2 / 100)).toFixed(2)}\\)`, 
			`\\( = £${value1Modified_0.toFixed(2)}\\)`, 
			
			`Year 2: £\\(${value1Modified_0}\\) + \\(${value2}\\)%`, 
			`\\( = £${value1Modified_0} + £${(value1Modified_0 * (value2 / 100)).toFixed(2)}\\)`, 
			`\\( = £${value1Modified_1.toFixed(2)}\\)`, 

			`Year 3: £\\(${value1Modified_1}\\) + \\(${value2}\\)%`, 
			`\\( = £${value1Modified_1} + £${(value1Modified_1 * (value2 / 100)).toFixed(2)}\\)`, 
			`\\( = £${value1Modified_2.toFixed(2)}\\)`
		];
		return steps;
	}
	//////// HTML ELEMENTS ////////
